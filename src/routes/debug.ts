...
/**
 * TEMPORARY diagnostic route. Lets us probe Salla's API with any merchant's
 * token without having to expose tokens in chat. Gated by a query-string key.
 *
 * Usage:
 *   GET  /api/_debug/salla?key=...&store=<storeId>&path=/store/info
 *   POST /api/_debug/salla?key=...&store=<storeId>&path=/some/endpoint
 *     body forwarded as-is
 *
 * DELETE this file after investigating.
 */
